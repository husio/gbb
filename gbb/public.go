...
// StyleHandler returns a HTTP handler that serves global CSS. If minify flag
// is set, CSS is minimized before serving.
func StyleHandler(minify bool) surf.HandlerFunc {
	now := time.Now()
...
body                     { margin: 40px auto; max-width: 1000px; line-height: 180%; padding: 0 10px;
                           font-family: "Open Sans",arial,x-locale-body,sans-serif; font-size: 15px; }
textarea,
input[type=text],
...
img.avatar               { border-radius: 50%; float: left; margin-right: 8px; }
`
